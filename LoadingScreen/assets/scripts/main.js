...
function setup() {
    ...
    // Rules
    Config.Rules.forEach((rules, index) => {
        $(".categories .rules").append(`<div class="box" data-id="${rules.name}" data-link="${rules.link}"><img class="icon" src="${rules.icon}"><div class="info"><p class="title">${rules.label}</p><p class="description">${rules.description}</p></div></div>`)
    });

    var copyTimeouts = {};
    $(".categories .rules .box").on("click", function() {
        let id = $(this).data("id")
